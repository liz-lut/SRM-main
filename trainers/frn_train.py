...
def default_train(train_loader,model,optimizer,writer,iter_counter):
    ...
    for i, (inp,_) in enumerate(train_loader):
        ...
        y = torch.from_numpy(np.repeat(range( way ), query_shot ))
        y_oh = 1 - torch.zeros((len(y), way)).scatter_(1, y.unsqueeze(1), 1)
        y_oh = y_oh.cuda()
        frn_loss_dual = -1 * (y_oh * log_prediction).sum(-1).mean()
         
        
        frn_loss = criterion(log_prediction,target)
        aux_loss = auxrank(s)
        loss = frn_loss + aux_loss + 0.001 * frn_loss_dual
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
